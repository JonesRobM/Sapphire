...
class Gyration():
    ...
    def calculate(self):
        ...
        if self.Type == 'Homo':
            Distances = CoMDist(CoM = get_CoM(get_subspecieslist(self.Metal, self.Elements, self.Positions)), 
                                 homo=True, positions = self.Positions, specie = self.Metal, elements=self.Elements)
    ...
    def write(self):
        ...
        if self.Type == 'Full':
            ...
            self.MakeFile(Attributes)
            with open(OutFile, 'a') as outfile:
                outfile.write(str(self.Frame) + ' ' +  ' '.join(str(item) for item in self.Gyrate) +'\n')
...
class Stat_Radius():
    ...
    def write(self):
        ...
        self.MakeFile(Attributes)
        with open(OutFile, 'a') as outfile:
            outfile.write(str(self.Frame) + ' ' +  ' '.join(str(item) for item in self.Rad) +'\n')
